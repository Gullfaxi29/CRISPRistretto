...
def main():
    try:
        ...
        description = ['~~~CRISPRessoPooled~~~', '-Analysis of CRISPR/Cas9 outcomes from POOLED deep sequencing data- (But less annoying)']
        pooled_string = "_____"
        ...
        if len(sys.argv) == 1:
            print(CRISPRessoShared.format_cl_text('usage: CRISPRistrettoPooled [-r1 FASTQ_R1] [-r2 FASTQ_R2] [-f AMPLICONS_FILE] [-x GENOME_ROOT] [-n NAME]\n' + \
        ...
        OUTPUT_DIRECTORY = 'CRISPRistrettoPooled_on_{0}'.format(normalize_name(args.name, args.fastq_r1, args.fastq_r2, args.aligned_pooled_bam))
        ...
        #--------------------------------------------------------------------------------------------------------------------------------------------------
        #check files
        #We are doing all our preprocessing seperately so aligned_pooled_bam is NOT OPTIONAL
        if args.aligned_pooled_bam is not None:
            ...
            parser.print_help()
            raise CRISPRessoShared.BadParameterException('Please provide input data for pooled analysis e.g. using the --aligned_pooled_bam parameter.')
